...
yaml_file = open(os.getenv('HMPRACTICEPY_YAML', ''))
settings = yaml.load(yaml_file)
yaml_file.close()

host = settings['HMPRACTICEPY_DB_HOST']
database = settings['HMPRACTICEPY_DB']
maxConnections = settings['HMPRACTICEPY_DB_MAX_CONNECTIONS']
user = settings['HMPRACTICEPY_DB_USER']
password = settings['HMPRACTICEPY_DB_PASSWORD']
maxTries = settings['HMPRACTICEPY_DB_MAX_TRIES']

psycopg2.extensions.register_type(psycopg2.extensions.UNICODE)
conn_pool = pool.ThreadedConnectionPool(minconn=1, maxconn=maxConnections, host=host, database=database, user=user,
                                        password=password)
